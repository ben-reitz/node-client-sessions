...
function hmacInit(algo, key) {
	...
	const drop = match[2] ? Number.parseInt(match[2], 10) : 0;
...
function decode(opts, content) {
	...
	const createdAt = Number.parseInt(components[2], 10);
	const duration = Number.parseInt(components[3], 10);
...
function Session(req, res, cookies, opts) {
	...
	const secure = res.socket?.encrypted || req.connection?.proxySecure;
...
Session.prototype = {
	...
	unbox: function (content) {
		...
		for (const k of Object.keys(unboxed.content)) {
			this._content[k] = unboxed.content[k];
		}
...
function clientSessionFactory(opts) {
	...
	if (!ENCRYPTION_ALGORITHMS[encAlg]) {
		throw new Error(
			`invalid encryptionAlgorithm, supported are: ${Object.keys(ENCRYPTION_ALGORITHMS).join(", ")}`,
		);
	}
	...
	if (!SIGNATURE_ALGORITHMS[sigAlg]) {
		throw new Error(
			`invalid signatureAlgorithm, supported are: ${Object.keys(SIGNATURE_ALGORITHMS).join(", ")}`,
		);
	}
    ...
    */
	...
	return function clientSession(req, res, next) {
		...
		if (opts.cookie.secureProxy) {
			...
			const { secureProxy, ...cookie } = opts.cookie;
			opts.cookie = cookie;
		}
