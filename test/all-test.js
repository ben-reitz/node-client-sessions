	...
	const browser = {
		get: (url, options, callback) => {
			...
			if (server && !server.__listening) {
				(server.__deferred = server.__deferred || []).push([
					url,
					options,
					callback,
				]);
				if (!server.__started) {
					server.__listener = server.listen(
						(server.__port = ++startingPort),
						"127.0.0.1",
						() => {
							process.nextTick(() => {
								server.__deferred.forEach((args) => {
...
suite.addBatch({
	middleware: {
		...
		"includes a session object": (err, req) => {
			assert.isObject(req.session);
		},
		"session object stores and retrieves values properly": (err, req) => {
			req.session.foo = "bar";
			assert.equal(req.session.foo, "bar");
		},
		"session object has reset function": (err, req) => {
			assert.isFunction(req.session.reset);
		},
		"session object has setDuration function": (err, req) => {
			assert.isFunction(req.session.setDuration);
		},
		"set variables and clear them yields no variables": (err, req) => {
			req.session.bar = "baz";
			req.session.reset();
			assert.isUndefined(req.session.bar);
		},
		"set variables does the right thing for Object.keys": (err, req) => {
			req.session.reset();
			req.session.foo = "foobar";
			assert.equal(Object.keys(req.session).length, 1);
			assert.equal(Object.keys(req.session)[0], "foo");
		},
		"reset preserves variables when asked": (err, req) => {
			req.session.reset();
			req.session.foo = "foobar";
		...
		"set session property absorbs set object": (err, req) => {
			req.session.reset();
...
suite.addBatch({
	"across two requests": {
		...
		"session maintains state": (err, req) => {
			assert.equal(req.session.foo, "foobar");
...
suite.addBatch({
	"across two requests": {
		...
		"resetting a session with an existing cookie value yields no variables": (
			err,
			req,
		) => {
			req.session.reset();
...
suite.addBatch({
	...
	"reading from a non-existing session": {
		...
		"does not set a cookie": (err, res, body) => {
			assert.isUndefined(res.headers["set-cookie"]);
...
suite.addBatch({
	"writing to a session": {
		...
		"sets a cookie": (err, res) => {
			assert.isArray(res.headers["set-cookie"]);
		},
		"and only one cookie": (err, res) => {
			assert.equal(res.headers["set-cookie"].length, 1);
		},
	},
});
...
suite.addBatch({
	"querying within duration": {
		...
		"session still has state": (err, req) => {
			assert.equal(req.session.foo, "foobar");
		},
	},
});
...
suite.addBatch({
	"modifying the session": {
		...
		"doesn't change createdAt": (err, req) => {
			assert.equal(req.session.foo, "foobar");
		},
	},
});
...
suite.addBatch({
	"querying outside the duration time": {
		...
		"session no longer has state": (err, req) => {
			assert.isUndefined(req.session.foo);
		},
	},
});
...
suite.addBatch({
	"after changing cookie duration and querying outside the modified duration": {
		topic: function () {
			...
			var browser = createBrowser(app);
			browser.get("/create", (res, $) => {
				browser.get("/change", (res, $) => {
					setTimeout(() => {
						browser.get("/complete", (res, $) => {
							browser.done();
						});
					}, 700);
				});
...
suite.addBatch({
	"changing duration": {
		...
		"doesn't affect session variables": (err, req) => {
			assert.equal(req.session.foo, "foo");
		},
		"does update creation time": (err, req) => {
			assert.notEqual(
				initialCookie.split(".")[2],
				updatedCookie.split(".")[2],
				"after duration update, creation should be updated",
			);
		},
		"does update duration": (err, req) => {
...
suite.addBatch({
	"after changing duration then setting a new session variable": {
		topic: function () {
			...
			var app = create_app_with_duration_modification();
			app.get("/set_then_duration", (req, res) => {
				req.session.baz = "baz";
				req.session.setDuration(500);
				res.send("did it");
			});
			...
			var browser = createBrowser(app);
			browser.get("/create", (res, $) => {
				browser.get("/set_then_duration", (res, $) => {
					browser.get("/complete", (res, $) => {
						browser.done();
					});
				});
...
suite.addBatch({
	"setting new variables then invoking setDuration": {
		...
		"both variables are visible": (err, req) => {
			assert.equal(req.session.foo, "foo");
...
function create_app_with_secure(firstMiddleware) {
	...
	var app = express();
	if (firstMiddleware) app.use(firstMiddleware);
...
suite.addBatch({
	"across two requests, with proxySecure, secure cookies": {
		topic: function () {
			...
			var app = create_app_with_secure((req, res, next) => {
...
suite.addBatch({
	"public encode and decode util methods": {
		...
		"encode ": (err, req) => {
			var result = cookieSessions.util.encode(
				{ cookieName: "session", secret: "yo" },
				{ foo: "bar" },
			);
			var result_arr = result.split(".");
			assert.equal(result_arr.length, 5);
		},
		"encode and decode - is object": (err, req) => {
			var encoded = cookieSessions.util.encode(
				{ cookieName: "session", secret: "yo" },
				{ foo: "bar" },
			);
			var decoded = cookieSessions.util.decode(
				{ cookieName: "session", secret: "yo" },
				encoded,
			);
			assert.isObject(decoded);
		},
		"encode and decode - has all values": (err, req) => {
			var encoded = cookieSessions.util.encode(
				{ cookieName: "session", secret: "yo" },
				{ foo: "bar", bar: "foo" },
			);
		...
		"encode and decode - override duration and createdAt": (err, req) => {
			var encoded = cookieSessions.util.encode(
				{ cookieName: "session", secret: "yo" },
				{ foo: "bar", bar: "foo" },
				5000,
		...
		"encode and decode - default duration": (err, req) => {
			var encoded = cookieSessions.util.encode(
				{ cookieName: "session", secret: "yo" },
				{ foo: "bar" },
			);
		...
		"encode and decode - tampered HMAC": (err, req) => {
			var encodedReal =
				"LVB3G2lnPF75RzsT9mz7jQ.RT1Lcq0dOJ_DMRHyWJ4NZPjBXr2WzkFcUC4NO78gbCQ.1371704898483.5000.ILEusgnajT1sqCWLuzaUt-HFn2KPjYNd38DhI7aRCb9";
			var encodedFake = encodedReal.substring(0, encodedReal.length - 1) + "A";

			var decodedReal = cookieSessions.util.decode(
				{ cookieName: "session", secret: "yo" },
				encodedReal,
			);
			assert.isObject(decodedReal);
			var decodedFake = cookieSessions.util.decode(
				{ cookieName: "session", secret: "yo" },
				encodedFake,
			);
			assert.isUndefined(decodedFake);
		},
		"decode - invalid input": (err, req) => {
			var notEnoughComponents =
				"LVB3G2lnPF75RzsT9mz7jQ.RT1Lcq0dOJ_DMRHyWJ4NZPjBXr2WzkFcUC4NO78gbCQ.1371704898483.5000";
			assert.isUndefined(
				cookieSessions.util.decode(
					{ cookieName: "session", secret: "yo" },
					notEnoughComponents,
				),
			);
			...
			var invalidBase64 =
				"LVB3G2lnPF75RzsT9mz7jQ.RT1Lcq0dOJ_DMRHyWJ4NZPjBXr2WzkFcUC4NO78gb.1371704898483.5000.ILEusgnajT1sqCWLuzaUt-HFn2KPjYNd38DhI7aRCb9";
			assert.isUndefined(
				cookieSessions.util.decode(
					{ cookieName: "session", secret: "yo" },
					invalidBase64,
...
suite.addBatch({
	"two middlewares": {
		...
		"We can write to both stores": (err, req) => {
			req.session.foo = "bar";
...
suite.addBatch({
	"specifying requestKey different than cookieName": {
		...
		"session is defined as req[requestKey]": (err, req) => {
	...
	"swapping two cookies": {
		topic: function () {
			...
			createBrowser(app).get("/foo", (res, $) => {
				...
				var firstHijack =
					getCookieName(firstCookie) + getCookieValue(secondCookie);
				var secondHijack =
					getCookieName(secondCookie) + getCookieValue(firstCookie);
		...
		"doesn't keep using cookie": (err, req) => {
...
suite.addBatch({
	"missing cookie maxAge": {
		...
		"still has an expires attribute": (err, res) => {
			assert.match(
		...
		"which roughly matches the session duration": (err, res) => {
			var expiryValue = res.headers["set-cookie"][0].replace(
				/^.*expires=([^;]+);.*$/,
				"$1",
			);
			...
			assert(
				Math.abs(50000 - cookieDuration) < 1500,
				"expiry is pretty far from the specified duration",
			);
		},
	},
	"changing the duration": {
		...
		"updates the cookie expiry": (err, res) => {
			var expiryValue = res.headers["set-cookie"][0].replace(
				/^.*expires=([^;]+);.*$/,
				"$1",
			);
			...
			var cookieDuration = expiryDate.getTime() - Date.now();
			assert(
				Math.abs(cookieDuration - 5000) < 1000,
				"expiry is pretty far from the specified duration",
			);
		},
	},
	"active user with session close to expiration": {
		topic: function () {
			...
			browser.get("/foo", () => {
				browser.get("/bar", () => {
					setTimeout(() => {
						browser.get("/baz", { json: true }, (res, first) => {
							setTimeout(() => {
								browser.get("/baz", { json: true }, (res, second) => {
									browser.done();
		...
		"extends session duration": (err, extended, tooLate) => {
...
suite.addBatch({
	"non-ephemeral cookie": {
		topic: function () {
			...
			shared_browser1.get("/foo", (res, $) => {
				self.callback(null, res);
			});
		},
		"has an expires attribute": (err, res) => {
			assert.match(
				res.headers["set-cookie"][0],
				/expires/,
				"cookie is a session cookie",
		...
		"changing to an ephemeral one": {
			topic: function () {
				var self = this;
				shared_browser1.get("/bar", (res, $) => {
					shared_browser1.done();
					self.callback(null, res);
				});
			},
			"removes its expires attribute": (err, res) => {
				assert.strictEqual(
					res.headers["set-cookie"][0].indexOf("expires="),
					-1,
					"cookie is not ephemeral",
				);
			},
		},
	},
	"ephemeral cookie": {
		topic: function () {
			...
			shared_browser2.get("/foo", (res, $) => {
				self.callback(null, res);
			});
		},
		"doesn't have an expires attribute": (err, res) => {
			assert.strictEqual(
				res.headers["set-cookie"][0].indexOf("expires="),
				-1,
				"cookie is not ephemeral",
		...
		"changing to an non-ephemeral one": {
			topic: function () {
				var self = this;
				shared_browser2.get("/bar", (res, $) => {
					shared_browser2.done();
					self.callback(null, res);
				});
			},
			"gains an expires attribute": (err, res) => {
				assert.match(
					res.headers["set-cookie"][0],
					/expires/,
					"cookie is a session cookie",
...
suite.addBatch({
	"sameSite cookie": {
		...
		"has samesite attribute": (err, res) => {
			assert.match(
				res.headers["set-cookie"][0],
				/samesite=lax/,
...
function testHmac(algo) {
	...
	block.topic = () => {
		...
		return cookieSessions.util
			.computeHmac(opts, iv, ciphertext, duration, createdAt)
			.toString("base64");
	};

	block["equals test vector"] = (val) => {
